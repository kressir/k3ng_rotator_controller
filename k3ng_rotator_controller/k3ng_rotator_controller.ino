...
#include <FlashStorage.h>  //the SAMD21 does not have eeprom and this lib will emulate eeprom on flash storage - danger here is that settings are erased at every flash
#include <avr/dtostrf.h> 
//#include <dtostrf.h>
#include <Sodaq_wdt.h>  //the Sodaq_wdt watchdog timer library may work for both - not tested though
...
#include "rotator_settings.h"
	...
	#ifdef ARDUINO_SAMD_ZERO
		#include <Adafruit_ZeroTimer.h>  // included in avdweb_SAMDtimer
...
USB_SERIAL_PORT_CLASS * control_port;
#else
...
long lastHzCheck = 0.0;
long lastIntTimerCount = 0.0;
void loop() {
...
/*void update_display() {
	...
	k3ngdisplay.print_top_left("Test");
	k3ngdisplay.service(1);
	debug.println("Display test");
	SerialUSB.flush();
}*/
...
void initialize_eeprom_with_defaults() {
	...
	configuration.clock_timezone_offset = -6; //0;
...
void read_elevation(byte force_read) {
	...
	if (1) {
		...
		elevation = (atan2(lsm.accelData.x, lsm.accelData.z) * 180 * HEADING_MULTIPLIER) / M_PI;
#ifdef FEATURE_ELEVATION_CORRECTION
		elevation = (correct_elevation(elevation / (float)HEADING_MULTIPLIER) * HEADING_MULTIPLIER);
#endif // FEATURE_ELEVATION_CORRECTION
		elevation = elevation + (configuration.elevation_offset * HEADING_MULTIPLIER);
...
void initialize_interrupts() {
	...
	int TIMER_PRESCALER_DIV = 1; // should match above
	float frequencyHz = 1000000.0/STEPPER_TIMER_PERIOD_MICROSECONDS;  //with prescaler = 1, range is about 735 Hz to 48,000,000 Hz  (1360 to 0.0208)
	int compareValue = (48e6 / (TIMER_PRESCALER_DIV * frequencyHz)) - 1.0;
	ZeroTimer.setPeriodMatch(compareValue, compareValue, TC_CALLBACK_CC_CHANNEL0);  // use for TC_WAVE_GENERATION_MATCH
	ZeroTimer.setCallback(true, TC_CALLBACK_CC_CHANNEL0, service_stepper_motor_pulse_pins);
	ZeroTimer.enable(true);
	#else
		#ifdef OPTION_STEPPER_MOTOR_USE_TIMER_ONE_INSTEAD_OF_FIVE
			//Timer1.initialize(250);  // 250 us = 4 khz rate
			Timer1.initialize(STEPPER_TIMER_PERIOD_MICROSECONDS);  // 25 us = 40 khz rate - faster for teensy and microstepping
			Timer1.attachInterrupt(service_stepper_motor_pulse_pins);
		#else
			Timer5.initialize(STEPPER_TIMER_PERIOD_MICROSECONDS);  // 250 us = 4 khz rate
...
void service_rotation() {
	...
	if (az_state == INITIALIZE_NORMAL_CW) {
		...
		debug.print("service_rotation: INITIALIZE_NORMAL_CW -> NORMAL_CW");
	...
	if (az_state == INITIALIZE_NORMAL_CCW) {
		...
		az_state = NORMAL_CCW;
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_NORMAL_CCW -> NORMAL_CCW");
#endif // DEBUG_SERVICE_ROTATION
	}
	...
	if (az_state == INITIALIZE_SLOW_START_CW) {
		...
		rotator(ACTIVATE, CW);
		az_slowstart_start_time = millis() - (mapf(current_az_speed_voltage, 0, normal_az_speed_voltage, 0, AZ_SLOW_START_UP_TIME));
		az_last_step_time = 0;
		az_slow_start_step = mapf(current_az_speed_voltage, 0, normal_az_speed_voltage, 0, AZ_SLOW_START_STEPS);
	...
	if (az_state == INITIALIZE_SLOW_START_CCW_RESUME) {
		rotator(ACTIVATE, CCW);
		az_slowstart_start_time = millis() - (mapf(current_az_speed_voltage, 0, normal_az_speed_voltage, 0, AZ_SLOW_START_UP_TIME));
		az_last_step_time = 0;
		az_slow_start_step = mapf(current_az_speed_voltage, 0, normal_az_speed_voltage, 0, AZ_SLOW_START_STEPS);
		az_state = SLOW_START_CCW;
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_SLOW_START_CCW_RESUME -> SLOW_START_CCW");
#endif // DEBUG_SERVICE_ROTATION
	}

	if (az_state == INITIALIZE_SLOW_START_CW_RESUME) {
		rotator(ACTIVATE, CW);
		az_slowstart_start_time = millis() - (mapf(current_az_speed_voltage, 0, normal_az_speed_voltage, 0, AZ_SLOW_START_UP_TIME));
		az_last_step_time = 0;
		az_slow_start_step = mapf(current_az_speed_voltage, 0, normal_az_speed_voltage, 0, AZ_SLOW_START_STEPS);
		az_state = SLOW_START_CW;
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_SLOW_START_CW_RESUME -> SLOW_START_CW");
#endif // DEBUG_SERVICE_ROTATION
	}

	if (az_state == INITIALIZE_TIMED_SLOW_DOWN_CW) {
		az_direction_change_flag = 0;
		az_timed_slow_down_start_time = millis() - (mapf(current_az_speed_voltage, 0, normal_az_speed_voltage, TIMED_SLOW_DOWN_TIME, 0));
		az_last_step_time = millis();
		az_slow_down_step = mapf(current_az_speed_voltage, 0, normal_az_speed_voltage, AZ_SLOW_DOWN_STEPS, 0);
		az_state = TIMED_SLOW_DOWN_CW;
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_TIMED_SLOW_DOWN_CW -> TIMED_SLOW_DOWN_CW");
#endif // DEBUG_SERVICE_ROTATION
	}
...
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_TIMED_SLOW_DOWN_CCW -> TIMED_SLOW_DOWN_CCW");
#endif // DEBUG_SERVICE_ROTATION
	}
	...
	if (az_state == INITIALIZE_DIR_CHANGE_TO_CW) {
		...
		az_timed_slow_down_start_time = millis() - (mapf(current_az_speed_voltage, 0, normal_az_speed_voltage, TIMED_SLOW_DOWN_TIME, 0));
		az_last_step_time = millis();
		az_slow_down_step = mapf(current_az_speed_voltage, 0, normal_az_speed_voltage, 0, AZ_SLOW_DOWN_STEPS);
		az_state = TIMED_SLOW_DOWN_CCW;
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_DIR_CHANGE_TO_CW -> TIMED_SLOW_DOWN_CCW");
#endif // DEBUG_SERVICE_ROTATION
	}
	...
	if (az_state == INITIALIZE_DIR_CHANGE_TO_CCW) {
		...
		az_state = TIMED_SLOW_DOWN_CW;
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_DIR_CHANGE_TO_CCW -> TIMED_SLOW_DOWN_CW");
#endif // DEBUG_SERVICE_ROTATION
	}
	...
	if ((az_state == SLOW_START_CW) || (az_state == SLOW_START_CCW)) {
		if ((millis() - az_slowstart_start_time) >= AZ_SLOW_START_UP_TIME) {  // is it time to end slow start?
			...
			if (((millis() - az_last_step_time) > (AZ_SLOW_START_UP_TIME / AZ_SLOW_START_STEPS)) && (normal_az_speed_voltage > AZ_SLOW_START_STARTING_PWM)) {
				int speedToBeAt = mapf(millis() - az_slowstart_start_time, 0, AZ_SLOW_START_UP_TIME, 0, normal_az_speed_voltage);
				speedToBeAt = constrain(speedToBeAt, AZ_SLOW_START_STARTING_PWM, normal_az_speed_voltage);
				//time since slow start started, mapped to voltage from low to high

#ifdef DEBUG_SERVICE_ROTATION
				debug.print("service_rotation: step up: ");
				debug.print(az_slow_start_step);
				debug.print(" pwm: ");
				debug.print(speedToBeAt);
				debug.println("");
#endif // DEBUG_SERVICE_ROTATION
				...
				update_az_variable_outputs(speedToBeAt);
				az_last_step_time = millis();
				az_slow_start_step = mapf(constrain(millis() - az_slowstart_start_time, 0, AZ_SLOW_START_UP_TIME), 0, AZ_SLOW_START_UP_TIME, 0, AZ_SLOW_START_STEPS);
				// instead of incrementing or decrementing steps, calculate where we should be based on how much time has passed
			}
		}
	...
	if (((az_state == TIMED_SLOW_DOWN_CW) || (az_state == TIMED_SLOW_DOWN_CCW)) && ((millis() - az_last_step_time) >= (TIMED_SLOW_DOWN_TIME / AZ_SLOW_DOWN_STEPS))) {
		...
		int speedToBeAt = mapf(millis() - az_timed_slow_down_start_time, 0, TIMED_SLOW_DOWN_TIME, normal_az_speed_voltage, 0);
		speedToBeAt = constrain(speedToBeAt, AZ_SLOW_DOWN_PWM_STOP, normal_az_speed_voltage);
		//time since slow started, mapped to voltage from high to low
		...
		update_az_variable_outputs(speedToBeAt);  
		az_last_step_time = millis();
		az_slow_down_step = mapf(constrain(millis() - az_timed_slow_down_start_time, 0, TIMED_SLOW_DOWN_TIME), 0, TIMED_SLOW_DOWN_TIME, AZ_SLOW_DOWN_STEPS, 0);
		// instead of incrementing or decrementing steps, calculate where we should be based on how much time has passed
	...
	if (el_state == INITIALIZE_NORMAL_UP) {
		...
		el_state = NORMAL_UP;
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_NORMAL_UP -> NORMAL_UP");
#endif // DEBUG_SERVICE_ROTATION
	}
...
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_NORMAL_DOWN -> NORMAL_DOWN");
#endif // DEBUG_SERVICE_ROTATION
	}
	...
	if (el_state == INITIALIZE_SLOW_START_UP) {
		...
		rotator(ACTIVATE, UP);
		el_slowstart_start_time = millis() - (mapf(current_el_speed_voltage, 0, normal_el_speed_voltage, 0, EL_SLOW_START_UP_TIME));
		el_last_step_time = 0;
		el_slow_start_step = mapf(current_el_speed_voltage, 0, normal_el_speed_voltage, 0, EL_SLOW_START_STEPS);
	...
	if (el_state == INITIALIZE_SLOW_START_DOWN_RESUME) {
		rotator(ACTIVATE, DOWN);
		el_slowstart_start_time = millis() - (mapf(current_el_speed_voltage, 0, normal_el_speed_voltage, 0, EL_SLOW_START_UP_TIME));
		el_last_step_time = 0;
		el_slow_start_step = mapf(current_el_speed_voltage, 0, normal_el_speed_voltage, 0, EL_SLOW_START_STEPS);
		el_state = SLOW_START_DOWN;
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_SLOW_START_DOWN_RESUME -> SLOW_START_DOWN");
#endif // DEBUG_SERVICE_ROTATION
	}

	if (el_state == INITIALIZE_SLOW_START_UP_RESUME) {
		rotator(ACTIVATE, UP);
		el_slowstart_start_time = millis() - (mapf(current_el_speed_voltage, 0, normal_el_speed_voltage, 0, EL_SLOW_START_UP_TIME));
		el_last_step_time = 0;
		el_slow_start_step = mapf(current_el_speed_voltage, 0, normal_el_speed_voltage, 0, EL_SLOW_START_STEPS);
		el_state = SLOW_START_UP;
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_SLOW_START_UP_RESUME -> SLOW_START_UP");
#endif // DEBUG_SERVICE_ROTATION
	}
	...
	if (el_state == INITIALIZE_TIMED_SLOW_DOWN_UP) {
		...
		/////0
		el_timed_slow_down_start_time = millis() - (mapf(current_el_speed_voltage, 0, normal_el_speed_voltage, TIMED_SLOW_DOWN_TIME, 0));
		el_last_step_time = millis();
		/////0
		el_slow_down_step = mapf(current_el_speed_voltage, 0, normal_el_speed_voltage, EL_SLOW_DOWN_STEPS, 0);
		el_state = TIMED_SLOW_DOWN_UP;
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_TIMED_SLOW_DOWN_UP -> TIMED_SLOW_DOWN_UP");
#endif // DEBUG_SERVICE_ROTATION
	}
...
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_TIMED_SLOW_DOWN_DOWN -> TIMED_SLOW_DOWN_DOWN");
#endif // DEBUG_SERVICE_ROTATION
	}
	...
	if (el_state == INITIALIZE_DIR_CHANGE_TO_UP) {
		...
		el_timed_slow_down_start_time = millis() - (mapf(current_el_speed_voltage, 0, normal_el_speed_voltage, TIMED_SLOW_DOWN_TIME, 0));
		el_last_step_time = millis();
		el_slow_down_step = mapf(current_el_speed_voltage, 0, normal_el_speed_voltage, 0, EL_SLOW_DOWN_STEPS);
		el_state = TIMED_SLOW_DOWN_DOWN;
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_DIR_CHANGE_TO_UP -> TIMED_SLOW_DOWN_DOWN");
#endif // DEBUG_SERVICE_ROTATION
	}
	...
	if (el_state == INITIALIZE_DIR_CHANGE_TO_DOWN) {
		...
		el_state = TIMED_SLOW_DOWN_UP;
#ifdef DEBUG_SERVICE_ROTATION
		debug.print("service_rotation: INITIALIZE_DIR_CHANGE_TO_DOWN -> TIMED_SLOW_DOWN_UP");
#endif // DEBUG_SERVICE_ROTATION
	}
	...
	if ((el_state == SLOW_START_UP) || (el_state == SLOW_START_DOWN)) {
		if ((millis() - el_slowstart_start_time) >= EL_SLOW_START_UP_TIME) {  // is it time to end slow start?
			...
			if (((millis() - el_last_step_time) > (EL_SLOW_START_UP_TIME / (EL_SLOW_START_STEPS))) && (normal_el_speed_voltage > EL_SLOW_START_STARTING_PWM)) {
				int speedToBeAt = mapf(millis() - el_slowstart_start_time, 0, EL_SLOW_START_UP_TIME, 0, normal_el_speed_voltage);
				speedToBeAt = constrain(speedToBeAt, EL_SLOW_START_STARTING_PWM, normal_el_speed_voltage);
				//time since slow start started, mapped to voltage from low to high
#ifdef DEBUG_SERVICE_ROTATION
				debug.print(" service_rotation: step up: ");
				...
				debug.println("");
#endif // DEBUG_SERVICE_ROTATION
				update_el_variable_outputs(speedToBeAt);
				el_last_step_time = millis();
				el_slow_start_step = mapf(constrain(millis() - el_slowstart_start_time, 0, EL_SLOW_START_UP_TIME), 0, EL_SLOW_START_UP_TIME, 0, EL_SLOW_START_STEPS);
				// instead of incrementing or decrementing steps, calculate where we should be based on how much time has passed
			}
		}
	...
	if (((el_state == TIMED_SLOW_DOWN_UP) || (el_state == TIMED_SLOW_DOWN_DOWN)) && ((millis() - el_last_step_time) >= (TIMED_SLOW_DOWN_TIME / EL_SLOW_DOWN_STEPS))) {
		int speedToBeAt = mapf(millis() - el_timed_slow_down_start_time, 0, TIMED_SLOW_DOWN_TIME, normal_el_speed_voltage, 0);
		speedToBeAt = constrain(speedToBeAt, EL_SLOW_DOWN_PWM_STOP, normal_el_speed_voltage);
		//time since slow started, mapped to voltage from high to low
		...
		update_el_variable_outputs(speedToBeAt);
		el_last_step_time = millis();
		el_slow_down_step = mapf(constrain(millis() - el_timed_slow_down_start_time,0, TIMED_SLOW_DOWN_TIME), 0, TIMED_SLOW_DOWN_TIME, EL_SLOW_DOWN_STEPS, 0);
		// instead of incrementing or decrementing steps, calculate where we should be based on how much time has passed
			...
			if (az_state != IDLE) {
				if (az_slowdown_active) {
					if ((az_state == TIMED_SLOW_DOWN_CW) || (az_state == TIMED_SLOW_DOWN_CCW) || (az_state == SLOW_DOWN_CW) || (az_state == SLOW_DOWN_CCW)) {  // if we're already in timed slow down and we get another stop, do a hard stop
						//changed -  use OPTION_BUTTON_RELEASE_NO_SLOWDOWN to enable stopping with no slow down (immediately, not on a second stop/button up) - that will call a REQUEST_KILL
						//rotator(DEACTIVATE, CW);
						//rotator(DEACTIVATE, CCW);
						//az_state = IDLE;
						//az_request_queue_state = NONE;
		...
		case (REQUEST_CW):
			...
			if (((az_state == SLOW_START_CCW) || (az_state == NORMAL_CCW) || (az_state == SLOW_DOWN_CCW) || (az_state == TIMED_SLOW_DOWN_CCW)) && (az_slowstart_active)) {
				...
				if ((az_state != SLOW_START_CW) && (az_state != NORMAL_CW)) {
					...
					if (az_slowstart_active) {
						if (az_state == TIMED_SLOW_DOWN_CW) {
							az_state = INITIALIZE_SLOW_START_CW_RESUME;
						}
						else {
							az_state = INITIALIZE_SLOW_START_CW;
						}
		...
		case (REQUEST_CCW):
			...
			if (((az_state == SLOW_START_CW) || (az_state == NORMAL_CW) || (az_state == SLOW_DOWN_CW) || (az_state == TIMED_SLOW_DOWN_CW)) && (az_slowstart_active)) {
				...
				if ((az_state != SLOW_START_CCW) && (az_state != NORMAL_CCW)) {
					...
					if (az_slowstart_active) {
						if (az_state == TIMED_SLOW_DOWN_CCW) {
							az_state = INITIALIZE_SLOW_START_CCW_RESUME;
						}
						else {
							az_state = INITIALIZE_SLOW_START_CCW;
						}
		...
		case (REQUEST_UP):
			...
			if (((el_state == SLOW_START_DOWN) || (el_state == NORMAL_DOWN) || (el_state == SLOW_DOWN_DOWN) || (el_state == TIMED_SLOW_DOWN_DOWN)) && (el_slowstart_active)) {
				...
				if ((el_state != SLOW_START_UP) && (el_state != NORMAL_UP)) {
					if (el_slowstart_active) {
						if (el_state == TIMED_SLOW_DOWN_UP) {
							el_state = INITIALIZE_SLOW_START_UP_RESUME;
						}
						else {
							el_state = INITIALIZE_SLOW_START_UP;
						}
		...
		case (REQUEST_DOWN):
			...
			if (((el_state == SLOW_START_UP) || (el_state == NORMAL_UP) || (el_state == SLOW_DOWN_UP) || (el_state == TIMED_SLOW_DOWN_UP)) && (el_slowstart_active)) {
				...
				if ((el_state != SLOW_START_DOWN) && (el_state != NORMAL_DOWN)) {
					if (el_slowstart_active) {
						if (el_state == TIMED_SLOW_DOWN_DOWN) {
							el_state = INITIALIZE_SLOW_START_DOWN_RESUME;
						}
						else {
							el_state = INITIALIZE_SLOW_START_DOWN;
						}
...
byte submit_remote_command(byte remote_command_to_send, byte parm1, int parm2) {
	...
	if ((remote_unit_command_submitted && ((remote_command_to_send == REMOTE_UNIT_AZ_COMMAND) || (remote_command_to_send == REMOTE_UNIT_EL_COMMAND) || (remote_command_to_send == REMOTE_UNIT_CL_COMMAND))) || suspend_remote_commands) {
		...
		case REMOTE_UNIT_DHL_COMMAND:
			...
			if (parm2 == HIGH) { strcat(ethernet_send_string, "H"); }
			else { strcat(ethernet_send_string, "L"); }
			parm1 = parm1 - 100;
			if (parm1 < 10) { strcat(ethernet_send_string, "0"); }
			dtostrf(parm1, 0, 0, temp_string);
			strcat(ethernet_send_string, temp_string);
		...
		case REMOTE_UNIT_DOI_COMMAND:
			...
			take_care_of_pending_remote_command();
			strcpy(ethernet_send_string, "D");
			if (parm2 == OUTPUT) { strcat(ethernet_send_string, "O"); }
			else { strcat(ethernet_send_string, "I"); }
			parm1 = parm1 - 100;
			if (parm1 < 10) { strcat(ethernet_send_string, "0"); }
			dtostrf(parm1, 0, 0, temp_string);
			strcat(ethernet_send_string, temp_string);
	...
	case '?':
		...
		if (input_buffer_index == 4) {
			if ((input_buffer[2] == 'F') && (input_buffer[3] == 'S')) {  // \?FS - Full Status
				...
				// RC
#ifdef FEATURE_GPS
				if (latitude < 0) { strcat(return_string, "-"); }
				else { strcat(return_string, "+"); }
				dtostrf(abs(latitude), 0, 4, temp_string);
				strcat(return_string, temp_string);
				strcat(return_string, ",");
				if (longitude < 0) { strcat(return_string, "-"); }
				else { strcat(return_string, "+"); }
...
#ifdef ARDUINO_SAMD_ZERO
				sodaq_wdt_enable(WDT_PERIOD_1DIV32);
#else
				wdt_enable(WDTO_30MS); 
#endif

				while (1) {}  //ZZZZZZ - TODO - change to reboot flag
			}
...
#ifdef ARDUINO_SAMD_ZERO
//void service_stepper_motor_pulse_pins(struct tc_module *const module_inst) {
...
void set_az_stepper_freq(unsigned int frequency) {
	...
	if (frequency > 0) {
		...
		az_stepper_freq_count = 2 * (1000000 / STEPPER_TIMER_PERIOD_MICROSECONDS) / frequency;
	}
	else {
...
void set_el_stepper_freq(unsigned long frequency) {


	if (frequency > 0) {
		//el_stepper_freq_count = 2000 / frequency;
		el_stepper_freq_count = 2*(1000000/ STEPPER_TIMER_PERIOD_MICROSECONDS) / frequency;
		//if timer running at 25 us, 40,000 hz (1,000,000/25) then ticks per step = 2*40000/frequency  (ticks/step)
	}
	else {
